// Se han extraído LATAM_COUNTRIES, keyNodes y layerData a data.js
        ...
        Object.values(LATAM_COUNTRIES).forEach(c => {
            // Limpieza de strings para cálculos matemáticos
            const gdpVal = parseFloat(c.gdp.replace(/,/g, '').replace('B USD', ''));
            const capitaVal = parseFloat(c.gdpCapita.replace(/,/g, '').replace(' USD', ''));
            const unempVal = parseFloat(c.unemp.replace('%', ''));

            totalGDP_B += gdpVal;
            const pop = (gdpVal * 1e9) / capitaVal; // Derivamos población real
            totalPop += pop;
        ...
        const projection = d3.geoMercator()
            .center([-70, -15])
            .scale(400)
            .translate([svg.node().parentNode.clientWidth / 2, svg.node().parentNode.clientHeight / 2]);
        ...
        d3.json("https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json").then(data => {
            ...
            // 1. Dibujar Capas (Invisibles inicialmente, detrás de los nodos clave)
        ...
        window.addEventListener('resize', () => {
            ...
            resizeTimeout = requestAnimationFrame(() => {
                ...
                projection.translate([container.clientWidth / 2, container.clientHeight / 2]);
                ...
                d3.selectAll(".layer-fdi")
                    .attr("cx", d => { d.coords = projection([d.lon, d.lat]); return d.coords[0]; })
                    .attr("cy", d => d.coords[1]);
                    
                d3.selectAll(".layer-tech")
                    .attr("x", d => { d.coords = projection([d.lon, d.lat]); return d.coords[0] - 4; })
                    .attr("y", d => d.coords[1] - 4);
                    
                d3.selectAll(".layer-res").attr("points", d => { const [x, y] = projection([d.lon, d.lat]); return `${x},${y - 6} ${x - 5},${y + 4} ${x + 5},${y + 4}`; });
                d3.selectAll(".layer-defensa").attr("points", d => { const [x, y] = projection([d.lon, d.lat]); return `${x},${y - 5} ${x + 5},${y} ${x},${y + 5} ${x - 5},${y}`; });

                d3.selectAll(".layer-tav")
                    .attr("x1", d => { d.p1 = projection([d[0].lon, d[0].lat]); return d.p1[0]; })
                    .attr("y1", d => d.p1[1])
                    .attr("x2", d => { d.p2 = projection([d[1].lon, d[1].lat]); return d.p2[0]; })
                    .attr("y2", d => d.p2[1]);

                d3.selectAll(".layer-strategic")
                    .attr("cx", d => { d.coords = projection([d.lon, d.lat]); return d.coords[0]; })
                    .attr("cy", d => d.coords[1]);

                d3.selectAll(".layer-infra").each(function(d) {
                    if(d.type === 'port') {
                        d3.select(this)
                            .attr("x", d => { d.coords = projection([d.lon, d.lat]); return d.coords[0] - 4; })
                            .attr("y", d => d.coords[1] - 4);
